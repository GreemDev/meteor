...
public abstract class ElytraFeatureRendererMixin<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    private Identifier modifyElytraTexture(Identifier original, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        if (livingEntity instanceof AbstractClientPlayerEntity playerEntity) {
            Identifier newElytra = Capes.get(playerEntity);
            if (newElytra != null) return newElytra;
        }
