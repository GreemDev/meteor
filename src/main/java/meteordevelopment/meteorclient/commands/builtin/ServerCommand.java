...
public class ServerCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<ClientCommandSource> builder) {
        ...
        builder.then(literal("tps").executes(ctx -> {
            float tps = TickRate.get();
            Formatting color = tps > 17.0f
                ? Formatting.GREEN
                : tps > 12.0f
                    ? Formatting.YELLOW
                    : Formatting.RED;
    ...
    private void basicInfo() {
        ...
        Set<String> brands = mc.player.getServer().getSaveProperties().getServerBrands();

        info("Type: %s", brands != null ? brands : "unknown");
