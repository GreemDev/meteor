...
public class NotebotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<ClientCommandSource> builder) {
        ...
        builder.then(literal("status").executes(ctx -> {
            info(Meteor.module(Notebot.class).getStatus());
            return SINGLE_SUCCESS;
        }));

        builder.then(literal("pause_or_resume").executes(ctx -> {
            Meteor.module(Notebot.class).pause();
            return SINGLE_SUCCESS;
        }));

        builder.then(literal("stop").executes(ctx -> {
            Meteor.module(Notebot.class).stop();
            return SINGLE_SUCCESS;
        }));

        builder.then(literal("randomsong").executes(ctx -> {
            Meteor.module(Notebot.class).playRandomSong();
            return SINGLE_SUCCESS;
        }));
        ...
        builder.then(
            literal("play").then(
                argument("song", NotebotSongArgumentType.create()).executes(ctx -> {
                    Path songPath = require(ctx.getArgument("song", Path.class), INVALID_SONG);
                    require(songPath.toFile().exists(), INVALID_SONG);

                    Meteor.module(Notebot.class).loadSong(songPath.toFile());
                    return SINGLE_SUCCESS;
                })
            )
        );
        ...
        builder.then(
            literal("preview").then(
                argument("song", NotebotSongArgumentType.create()).executes(ctx -> {
                    ...
                    Meteor.module(Notebot.class).previewSong(songPath.toFile());
                    return SINGLE_SUCCESS;
        })));
        ...
        builder.then(literal("record").then(literal("save").then(argument("name", StringArgumentType.greedyString()).executes(ctx -> {
            String name = require(ctx.getArgument("name", String.class), INVALID_SONG);
            require(!name.isEmpty(), INVALID_SONG);
