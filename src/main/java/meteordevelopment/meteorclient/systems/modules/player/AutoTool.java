...
public class AutoTool extends Module {
    ...
    private void onStartBreakingBlock(StartBreakingBlockEvent event) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            double score = getScore(itemStack, blockState, silkTouchForEnderChest.get(), fortuneForOresCrops.get(), prefer.get(), Predicate.not(this::shouldStopUsing));
            if (score < 0) continue;
        ...
        if ((bestSlot != -1 && (bestScore > getScore(currentStack, blockState, silkTouchForEnderChest.get(), fortuneForOresCrops.get(), prefer.get(), Predicate.not(this::shouldStopUsing))) || shouldStopUsing(currentStack) || !isTool(currentStack))) {
