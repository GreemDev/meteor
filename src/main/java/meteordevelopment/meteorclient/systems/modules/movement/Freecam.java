...
public class Freecam extends Module {
    ...
    private final Setting<Double> speed = sgGeneral.add(new DoubleSetting.Builder()
        ...
        .defaultValue(1.0)
        .onChanged(speed -> speedValue = speed)
        .range(1, 10)
        .build()
    );
    ...
    private final Setting<Double> sprintSpeed = sgGeneral.add(new DoubleSetting.Builder()
        .name("sprint-speed")
        .description("The speed to apply when holding your assigned sprint key while in freecam.\nThe default non-sprint movement speed factor is &z0.5&r.")
        .defaultValue(1)
        .range(0.6, 7.5)
        .build()
    );
    ...
    public Freecam() {
        super(Categories.Movement, "freecam", "Allows the camera to move away from the player.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        double s = 0.5;
        if (mc.options.sprintKey.isPressed()) s = sprintSpeed.get();
