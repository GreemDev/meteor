...
public class ServerSpoof extends Module {
    ...
    private class Listener {
        ...
        private void onPacketSend(PacketEvent.Send event) {
            if (!isActive()) return;
            if (!(event.packet instanceof CustomPayloadC2SPacket customPayload)) return;

            Identifier id = customPayload.payload().id();
            ...
            if (blockChannels.get()) {
                if (channels.get().stream().anyMatch(channel -> StringUtils.containsIgnoreCase(channel, id.toString())))
                    event.cancel();
            }
        }
