...
public class Surround extends Module {
    ...
    private void draw(Render3DEvent event, CardinalDirection direction, int y, int exclude) {
        ...
        var sideColor = getSideColor(renderPos);
        var lineColor = getLineColor(renderPos);
    ...
    public void onActivate() {
        ...
        if (toggleModules.get() && !modules.get().isEmpty() && Utils.canUpdate()) {
    ...
    public void onDeactivate() {
        if (toggleBack.get() && !toActivate.isEmpty() && Utils.canUpdate()) {
