...
public class BlockUtils {
    ...
    public static MobSpawn isValidMobSpawn(BlockPos blockPos, BlockState blockState, int spawnLightLevel) {
        ...
        if (!topSurface(downState)) {
            if (!downState.getCollisionShape(mc.world, down).equals(VoxelShapes.fullCube())) return MobSpawn.Never;
            if (downState.isTransparent(mc.world, down)) return MobSpawn.Never;
        }

        if (mc.world.getLightLevel(blockPos, 0) <= spawnLightLevel) return MobSpawn.Potential;
        else if (mc.world.getLightLevel(LightType.BLOCK, blockPos) <= spawnLightLevel) return MobSpawn.Always;
